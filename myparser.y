...
expression: operand			{ $$ = storeOperand($1); }
|	expression '+' expression  { $$ = arithmetical($1,'+',$3); }
|	expression '-' expression	{ $$ = arithmetical($1,'-',$3); }
|	expression '*' expression	{ printf("rule for * reached"); $$ = arithmetical($1,'*',$3); }
|	expression '/' expression	{ $$ = arithmetical($1,'/',$3); }
|	expression '%' expression	{ $$ = arithmetical($1,'%',$3); }
|	expression '^' expression	{ $$ = arithmetical($1,'^',$3); } //deleteable
|	len
//|	'{' list '}'
;
...
void initializeVar(struct varPack *pack){
	...
	int dir = assign_reg();	//DIR of variable
	int r1 = assign_reg();	//VALUE to store inside variable
...
void print(char* id){
	...
	if (var->dir != 0){
		...
		gc(line);
		snprintf(line, lineSize, "\tR2 = I(R6 - %x);\t//Generated by print\n", var->dir);		//R2 = I(dir);
		gc(line);
...
struct varPack * arithmetical(struct varPack* op1, int operation_sign, struct varPack* op2){
	...
	if ((op1->varType == 4 || op1->varType == 1) && 
		...
		/*snprintf(line,lineSize, "\tR%d=%d;%s\n",op1->varReg,op1->val_int, comment);
		gc(line);
		snprintf(line,lineSize, "\tR%d=%d;%s\n",op2->varReg,op2->val_int, comment);
		gc(line);*/
