...
void init_s_t() { /* iniciar tabla de símbolos */
	...
	voidp = top;
	insertar("-", tipo, entero, NULL);
	insertar("--", tipo, flotante, NULL);
	insertar("---", tipo, booleano, NULL);
	insertar("----", tipo, caracter, NULL);
}
...
void createVariable(struct node* var){
	if (buscar(var->id) == NULL){
		if (_scope == 0){
			insertar(var->id, v_global, var->tipo, NULL);
			return;
		}
		insertar(var->id, v_local, var->tipo, NULL);
	}
