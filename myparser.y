...
method: METH NAME 				{
									printf("yes b*tches");
									struct varPack *pack =  malloc(sizeof(struct varPack));
									pack->varReg = tag(); //una rutina almacenará su tag
									//pack->varType = $<my_type>8; //int
									pack->varCat = rutina;

									// PACK->VARREG ALMACENA TAG DE REGRESO A FUNCIÓN
									snprintf(line, lineSize, "L %d:\n// Generated by meth name at %d", pack->varReg, lines);
									gc(line);
									$<my_pack>$ = pack;
								}
		'('paramContainer')'	{	// Add params to inner scope
									
								} 
		':' type				{
									struct varPack *pack = $<my_pack>3;
									pack->varType = $<my_type>9;
									_globalgap++;
									insertar($2, rutina, $<my_type>8, _z - 4*_globalgap);
								}
		increaseScope content decreaseScope 	// RETURN isnt forced to be use
|	METH NAME '(' ')' ':' type	{
									_globalgap++;
									insertar($2, rutina, $<my_type>6, _z - 4*_globalgap);
								}
	increaseScope content decreaseScope
